...
void sample_texture_pixel(id<MTLTexture> texture, float x, float y, float *out_rgba);
...
typedef struct {
  ...
  bool mipmaps;
  MTLTextureUsage texture_usage;
  int backend;       // 0 = IMAGE_BACKEND_TEXTURE, 1 = IMAGE_BACKEND_BUFFER
  int bind_as;       // 0 = BIND_AS_TEXTURE, 1 = BIND_AS_BUFFER
  NSUInteger bind_index;
} ImageBufferConfig;
